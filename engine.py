...
class DownloadEngine:
    ...
    def start(self, job: DownloadJob, settings_snapshot):
        ...
        run_id = self.storage.record_run_start(job.run_label, len(job.ids), settings_snapshot)
    ...
    async def _process_video(self, session, video_id: int, job: DownloadJob):
        ...
        if job.max_bytes > 0 and probe.size and probe.size > job.max_bytes:
        ...
        if probe.content_type and "html" in probe.content_type.lower():
            self.app.emit_event(
                "diagnostic",
                {"message": f"Warning: {video_id} probe returned HTML content-type. Continuing download anyway."},
            )
    ...
    async def _request_with_retries(self, session, method, url, headers, proxy, allow_redirects, max_retries, unsafe_ssl, read_body=False):
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                request_kwargs["ssl"] = False if unsafe_ssl else None
